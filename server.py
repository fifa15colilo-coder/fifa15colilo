...
PACK_SETTINGS_TEMPLATE_PATH = ROOT / "pack_settings.json"
# User edits belong beside the local save. The game installation can be read-only
# and is overwritten by updates, while LocalAppData is writable and persistent.
PACK_SETTINGS_PATH = RUNTIME_ROOT / "pack_settings.json"
if not PACK_SETTINGS_PATH.exists() and PACK_SETTINGS_TEMPLATE_PATH.exists():
    try:
        shutil.copy2(PACK_SETTINGS_TEMPLATE_PATH, PACK_SETTINGS_PATH)
    except OSError as exc:
        log.warning("Could not create writable pack settings: %s", exc)
elif PACK_SETTINGS_PATH.exists() and PACK_SETTINGS_TEMPLATE_PATH.exists():
    try:
        editable = json.loads(PACK_SETTINGS_PATH.read_text(encoding="utf-8"))
        template = json.loads(PACK_SETTINGS_TEMPLATE_PATH.read_text(encoding="utf-8"))
        editable_packs = editable.setdefault("packs", {})
        changed = False
        # Retire the old one-off Josh test pack from existing runtime settings.
        for pid, current in list(editable_packs.items()):
            guaranteed_ids = [int(x) for x in current.get("guaranteed_resource_ids", []) or [] if str(x).isdigit()]
            if "josh 99" in str(current.get("name", "")).lower() or 167792961 in guaranteed_ids:
                editable_packs.pop(pid, None)
                changed = True
            elif ("legends prototype" in str(current.get("name", "")).lower()
                  or "pelé legend test" in str(current.get("name", "")).lower()
                  or "pelé identity test" in str(current.get("name", "")).lower()
                  or 201551957 in guaranteed_ids):
                # v41/v42 encoded Pelé as a special resource ID. FIFA rendered
                # the Legend background but could not resolve the player identity.
                editable_packs.pop(pid, None)
                changed = True
        for pid, definition in template.get("packs", {}).items():
            if pid not in editable_packs:
                editable_packs[pid] = definition
                changed = True
        wanted_next = max(int(editable.get("next_pack_id", 1)), int(template.get("next_pack_id", 1)))
        if int(editable.get("next_pack_id", 1)) != wanted_next:
            editable["next_pack_id"] = wanted_next
            changed = True
        if changed:
            temp_path = PACK_SETTINGS_PATH.with_suffix(".json.merge.tmp")
            temp_path.write_text(json.dumps(editable, indent=2) + "\n", encoding="utf-8")
            os.replace(temp_path, PACK_SETTINGS_PATH)
    except (OSError, ValueError, TypeError) as exc:
        log.warning("Could not merge new built-in packs into editable settings: %s", exc)

DEFAULT_SPECIAL_WEIGHTS = {"3":55,"4":4,"5":2,"6":3,"7":4,"8":12,"9":5,"11":8,"12":1}
...
def _merge_pack_settings(raw):
    ...
    # yet contain their own weights. Legends (12) are enabled by this build;
    # iMOTM (15) remains disabled because no matching client content is supplied.
...
# Aurora adds identity rows in this asset range.  FIFA can expose those raw
# asset IDs as zero-rated bronze base cards as well as the intended rarity-12
# resource IDs.  Only the encoded rarity-12 versions belong in Local FUT.
AURORA_LEGEND_ASSET_IDS = frozenset(
    int(meta.get("assetId", 0) or 0)
    for meta in PLAYER_DB.values()
    if int(meta.get("rareflag", 0) or 0) == 12
)

# The retail player catalogue already used many 225xxx IDs for low-rated base
# players. Aurora repurposes those same IDs as the underlying identities for
# its Legends. Leaving the old catalogue rows active makes a market search for
# (for example) Owen asset 225329 return both Icon Owen 201551921 and the old
# bronze resource 225329, which FIFA then renders with Owen's patched identity.
# Retire every colliding raw resource while retaining the encoded Icons.
for _raw_legend_id in AURORA_LEGEND_ASSET_IDS:
    _raw_meta = PLAYER_DB.get(int(_raw_legend_id))
    if _raw_meta is not None and int(_raw_meta.get("rareflag", 0) or 0) != 12:
        PLAYER_DB.pop(int(_raw_legend_id), None)
...
class State:
    ...
    def _init(self) -> None:
        with self.lock:
            ...
            # Remove stale raw/base copies of Aurora players.  Their resource
            # IDs equal the asset IDs (225xxx), whereas valid Icon cards use
            # the encoded rarity-12 IDs (201551xxx) and are never touched.
            if AURORA_LEGEND_ASSET_IDS:
                placeholders = ",".join("?" for _ in AURORA_LEGEND_ASSET_IDS)
                raw_ids = tuple(sorted(AURORA_LEGEND_ASSET_IDS))
                removed_items = c.execute(
                    f"DELETE FROM items WHERE resource_id IN ({placeholders})", raw_ids
                ).rowcount
                removed_auctions = c.execute(
                    f"DELETE FROM auctions WHERE resource_id IN ({placeholders})", raw_ids
                ).rowcount
                if removed_items or removed_auctions:
                    log.info(
                        "Removed %d duplicate bronze Legend item(s) and %d auction(s)",
                        removed_items, removed_auctions,
                    )
            self.conn.commit()
            self.set_default("credits", int(CFG["credits"]))
            self.set_default("fifa_points", int(CFG.get("fifa_points", 0)))
            ...
            if not self._has_any_items():
                self._seed_items()
            # Remove instances of the retired custom test player without
            # affecting normal cards, club identity, balances or records.
            retired_item_rows = self.conn.execute(
                "SELECT id FROM items WHERE resource_id=?", (167792961,)
            ).fetchall()
            for retired_row in retired_item_rows:
                retired_id = int(retired_row["id"])
                self.conn.execute("DELETE FROM auctions WHERE item_id=?", (retired_id,))
                self.conn.execute("DELETE FROM items WHERE id=?", (retired_id,))
            if retired_item_rows:
                self.conn.commit()
                log.warning("Removed %s retired custom test-card instance(s)", len(retired_item_rows))
    ...
    def fifa_points(self) -> int:
        return int(self.get("fifa_points", 0))

    def add_fifa_points(self, amount: int) -> int:
        v = max(0, self.fifa_points() + int(amount))
        self.set("fifa_points", v)
        return v
    ...
    def make_player_item(self, resource_id: int, pile: str = "club") -> dict[str, Any]:
        ...
        item = {
            ...
            "injuryType": 0 if int(resource_id) == 201551957 else "none",
            "injuryGames": 0,
            "lastSalePrice": 0,
            "fitness": 99,
            "training": 0,
            ...
            "displayName": str(meta.get("commonName", meta.get("name", ""))),
            "firstName": str(meta.get("firstName", "")),
            "lastName": str(meta.get("lastName", "")),
            "knownAs": str(meta.get("knownAs", meta.get("commonName", meta.get("name", "")))),
            "commonName": str(meta.get("commonName", meta.get("name", ""))),
            "dateOfBirth": str(meta.get("dateOfBirth", "")),
            "birthdate": str(meta.get("dateOfBirth", "")),
            "height": int(meta.get("height", 0) or 0),
            "weight": int(meta.get("weight", 0) or 0),
            "weakFoot": int(meta.get("weakFoot", 0) or 0),
            "skillMoves": int(meta.get("skillMoves", 0) or 0),
            "playStyle": 0,
            "discardValue": self.player_discard_value(resource_id),
            "itemType": "player",
            # The working Transfer Market Pelé definition uses subtype 0.
            # Subtype 1 makes the PC client draw the Legend shell but skip the
            # native player identity/metadata lookup.
            "cardsubtypeid": 0 if int(resource_id) == 201551957 else 1,
            "owners": 1,
            "untradeable": False,
            "morale": 50,
            "statsList": [],
            ...
            "assists": 0,
            "lifetimeAssists": 0,
            "loyaltyBonus": 0 if int(resource_id) == 201551957 else 1,
            "pile": pile,
            "nation": int(meta.get("nation", 0)),
            "nationId": int(meta.get("nation", 0)),
    ...
    def market_reference_price(self, resource_id: int) -> int:
        ...
        # Aurora/FIFA 15 Legends need their own economy.  Treating rarity 12
        # as a generic special made elite Icons appear for ordinary promo-card
        # prices.  These guides deliberately rise sharply with OVR, matching
        # the scarcity expected for the 57-card Legend catalogue.
        if rareflag == 12:
            legend_prices = {
                86: 350000,
                87: 500000,
            ...
            if rating <= 86:
                base = max(200000, 350000 - (86 - rating) * 50000)
            elif rating >= 95:
                base = min(15000000, 8000000 + (rating - 95) * 1500000)
            else:
                base = legend_prices[rating]
        elif rareflag == 1:
            base *= 1.18
        elif rareflag > 1:
    ...
    def create_user_auction(self, item_id: int, starting_bid: int, buy_now: int, duration: int) -> dict[str, Any] | None:
        ...
        # FIFA 15's retail UI normally derives a narrow per-card range from
        # MarketData. Local FUT deliberately permits any valid listing from
        # 150 through the local economy ceiling.
        listing_ceiling = 15_000_000
        starting_bid = min(listing_ceiling, max(150, int(starting_bid or 150)))
        buy_now = min(listing_ceiling, max(starting_bid, int(buy_now or starting_bid)))
    ...
    def simulate_ai_sales(self) -> None:
        ...
        with self.lock:
            ...
            for row in rows:
                ...
                starting_bid = int(row["starting_bid"] or 150)
                deterministic = (trade_id * 1103515245 + rid * 12345) & 0x7FFFFFFF
                ...
                bin_due = sellable and now >= created + auto_after

                # Listings above the instant-buy range can still attract a real
                # AI bid. Offers are always at least the stored market value and
                # may be up to 8% higher. They never exceed the user's Buy Now.
                if not bin_due and current_bid <= 0:
                    offer_delay = 25 + deterministic % 56
                    premium_percent = deterministic % 9
                    offer_value = int(market_value * (100 + premium_percent) / 100)
                    if offer_value >= 100000:
                        offer_value = int(round(offer_value / 1000.0) * 1000)
                    elif offer_value >= 10000:
                        offer_value = int(round(offer_value / 250.0) * 250)
                    else:
                        offer_value = int(round(offer_value / 50.0) * 50)
                    offer_value = min(ask, max(market_value, offer_value))
                    if starting_bid <= offer_value and now >= created + offer_delay:
                        current_bid = max(starting_bid, offer_value)
                        self.conn.execute(
                            "UPDATE auctions SET current_bid=? WHERE trade_id=?",
                            (int(current_bid), trade_id),
                        )
                        log.warning(
                            "AI MARKET OFFER trade=%s item=%s rid=%s bid=%s market=%s bin=%s",
                            trade_id, int(row["item_id"]), rid, current_bid, market_value, ask,
                        )

                bid_due = current_bid > 0 and now >= expires
                if bin_due or bid_due:
                    sold_price = ask if bin_due else current_bid
    ...
    def daily_reward_status(self) -> dict[str, Any]:
        today = _dt.date.today()
        last_text = str(self.get("daily_reward_last_date", "") or "")
        streak = max(0, int(self.get("daily_reward_streak", 0) or 0))
        last_date = None
        try:
            last_date = _dt.date.fromisoformat(last_text)
        except (TypeError, ValueError):
            pass
        claimed = last_date == today
        if not claimed and last_date != today - _dt.timedelta(days=1):
            streak = 0
        next_streak = streak if claimed else streak + 1
        rewards = (5, 6, 28, 29, 30, 31, 32)
        pack_id = rewards[(max(1, next_streak) - 1) % len(rewards)]
        pack_cfg = get_pack_cfg(pack_id)
        return {
            "claimedToday": claimed,
            "streak": streak,
            "nextStreak": next_streak,
            "packId": pack_id,
            "packName": str(pack_cfg.get("name", f"Pack {pack_id}")),
            "lastClaimDate": last_text,
        }

    def claim_daily_reward(self) -> dict[str, Any]:
        status = self.daily_reward_status()
        if status["claimedToday"]:
            return {**status, "claimed": False, "reason": "already_claimed"}
        if self.list_items("unassigned"):
            return {**status, "claimed": False, "reason": "unassigned_items"}
        pack = self.create_pack(int(status["packId"]))
        today_text = _dt.date.today().isoformat()
        self.set("daily_reward_last_date", today_text)
        self.set("daily_reward_streak", int(status["nextStreak"]))
        updated = self.daily_reward_status()
        return {
            **updated,
            "claimed": True,
            "numberItems": int(pack.get("numberItems", len(pack.get("itemData", [])))),
        }
    ...
    def create_pack(self, pack_id: int = 1) -> dict[str, Any]:
        ...
        legend_chance = max(0.0, min(1.0, float(pcfg.get("legend_chance", 0.0) or 0.0)))
        max_special_players = max(0, min(player_slots, int(pcfg.get("max_special_players", 2) or 0)))
        guaranteed_special_players = max(0, min(max_special_players, int(pcfg.get("guaranteed_special_players", 0) or 0)))
        guaranteed_min_rating = max(0, min(99, int(pcfg.get("guaranteed_min_rating", 0) or 0)))
        tier_weights = {
        ...
        def choose_legend():
            """Choose one unused Aurora Legend independently of promo odds."""
            candidates = []
            for tier_name in ("gold", "silver", "bronze"):
                for rid in player_pools[tier_name]["special"]:
                    if rid not in used_players and int(player_meta(rid).get("rareflag", 0) or 0) == 12:
            ...
            for tier_name in ("gold", "silver", "bronze"):
                try:
                    player_pools[tier_name]["special"].remove(rid)
                except ValueError:
                    pass
            return rid

        forced_resources: list[int] = []
        for raw_rid in pcfg.get("guaranteed_resource_ids", []) or []:
            try:
                rid = int(raw_rid)
            except (TypeError, ValueError):
                continue
            if rid in PLAYER_DB and rid not in used_players and len(forced_resources) < player_slots:
                forced_resources.append(rid)
                used_players.add(rid)

        # Guarantee one player at or above the configured rating, while leaving
        # the remaining slots governed by the normal tier/rare/special odds.
        if guaranteed_min_rating and player_slots > len(forced_resources):
            already_met = any(
                int(player_meta(rid).get("rating", 0) or 0) >= guaranteed_min_rating
                for rid in forced_resources
            )
            if not already_met:
                minimum_pool = [
                    int(rid) for rid in PLAYER_DB
                    if int(rid) not in used_players
                    and int(player_meta(int(rid)).get("rating", 0) or 0) >= guaranteed_min_rating
                ]
                if not minimum_pool:
                    raise RuntimeError(f"No player exists at or above rating {guaranteed_min_rating}")
                # Prefer a believable selection near the floor rather than
                # returning the single highest-rated card every time.
                minimum_pool.sort(key=lambda rid: int(player_meta(rid).get("rating", 0) or 0))
                window = minimum_pool[:min(80, len(minimum_pool))]
                rid = random.choice(window)
                forced_resources.append(rid)
                used_players.add(rid)

        player_selected: list[int] = list(forced_resources)
        special_players_selected = sum(
            1 for rid in forced_resources
            if bool(player_meta(rid).get("special", False)) or int(player_meta(rid).get("rareflag", 0) or 0) > 1
        )
        rare_slots_remaining = max(0, min(player_slots - len(player_selected), player_rare_slots - len(player_selected)))
        for _ in range(rare_slots_remaining):
            tier = choose_tier()
            allow_special = special_players_selected < max_special_players
            force_special = special_players_selected < guaranteed_special_players
            rid = choose_legend() if allow_special and legend_chance > 0 and random.random() < legend_chance else None
            if rid is None:
                rid = choose_special(tier) if allow_special and (force_special or random.random() < special_chance) else None
            if rid is None:
        ...
        common_slots_remaining = max(0, player_slots - len(player_selected))
        for _ in range(common_slots_remaining):
            tier = choose_tier()
            allow_special = special_players_selected < max_special_players
            force_special = special_players_selected < guaranteed_special_players
            rid = choose_legend() if allow_special and legend_chance > 0 and random.random() < legend_chance else None
        ...
        items = sorted(items, key=lambda x: (
            0 if str(x.get("itemType", "player")) == "player" else 1,
            -int(x.get("rating", 0) or 0), -int(x.get("rareflag", 0) or 0),
            -int(x.get("resourceId", x.get("assetId", 0)) or 0)
        ))
        ...
        log.warning("RETAIL PACK id=%s name=%s size=%d players=%d consumables=%d clubItems=%d rareSlots=%d specialChance=%.1f%% legendChance=%.1f%% guaranteedSpecials=%d maxSpecials=%d actualSpecials=%d",
                    pack_id, pcfg.get("name"), pack_size, player_count, consumable_count, club_count, rare_slots,
                    special_chance * 100, legend_chance * 100, guaranteed_special_players, max_special_players,
...
def user_mass_info() -> dict[str, Any]:
    return {
        "userInfo": {
            ...
            "credits": STATE.credits(),
            "established": 2014,
            "tradeOffersEnabled": True,
            "tradingEnabled": True,
            "feature": {
                "trade": 1,
                "trading": 1,
                "tradeOffers": 1,
                "tradeOffersEnabled": 1,
                "store": 1,
                "squad": 1,
                "friends": 0,
            },
...
def _native_player_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    meta = player_meta(resource_id)
    team_id = int(item.get("teamid", item.get("teamId", meta.get("teamid", 0))) or 0)
    if resource_id == 201551957:
        # Repair already-owned copies created by v44/v45 with Barcelona's ID.
        team_id = int(meta.get("teamid", 999) or 999)
    ...
    out = {
        ...
        "assetId": asset_id,
        # definitionId identifies the exact FUT card revision, not merely the
        # underlying footballer.  Using asset_id here made FIFA alternate
        # between the rarity-12 Icon and a raw bronze base card in market
        # results.  assetId remains available separately for face/identity.
        ...
        "itemType": "player",
        "lastSalePrice": int(item.get("lastSalePrice", 0) or 0),
        "leagueId": int(item.get("leagueId", meta.get("leagueId", 0)) or 0),
        ...
        "nation": int(item.get("nation", meta.get("nation", 0)) or 0),
        "nationId": int(item.get("nationId", item.get("nation", meta.get("nation", 0))) or 0),
        ...
        # FIFA's Player Bio view reparses these fields after an item has moved
        # from the pack pile into the club.  Preserve them here; dropping them
        # produced a valid Legend shell with a blank identity and 0'0 height.
        "name": str(item.get("name", meta.get("name", "")) or ""),
        "displayName": str(item.get("displayName", meta.get("displayName", meta.get("name", ""))) or ""),
        "firstName": str(item.get("firstName", meta.get("firstName", "")) or ""),
        "lastName": str(item.get("lastName", meta.get("lastName", "")) or ""),
        "knownAs": str(item.get("knownAs", item.get("commonName", meta.get("knownAs", meta.get("commonName", meta.get("name", ""))))) or ""),
        "commonName": str(item.get("commonName", item.get("knownAs", meta.get("commonName", meta.get("name", "")))) or ""),
        "dateOfBirth": str(item.get("dateOfBirth", item.get("birthdate", meta.get("dateOfBirth", ""))) or ""),
        "birthdate": str(item.get("birthdate", item.get("dateOfBirth", meta.get("dateOfBirth", ""))) or ""),
        "height": int(item.get("height", meta.get("height", 0)) or 0),
        "weight": int(item.get("weight", meta.get("weight", 0)) or 0),
        "weakFoot": int(item.get("weakFoot", item.get("weakfoot", meta.get("weakFoot", 0))) or 0),
        "weakfoot": int(item.get("weakfoot", item.get("weakFoot", meta.get("weakFoot", 0))) or 0),
        "skillMoves": int(item.get("skillMoves", item.get("skillmoves", meta.get("skillMoves", 0))) or 0),
        "skillmoves": int(item.get("skillmoves", item.get("skillMoves", meta.get("skillMoves", 0))) or 0),
    }
    if resource_id == 201551957:
        # Old persisted Pelé items also carried La Liga from the temporary
        # compatibility build; normalize those without deleting the club DB.
        out["leagueId"] = 2118
        out["cardsubtypeid"] = 0
        out["injuryType"] = 0
        out["loyaltyBonus"] = 0
...
def _store_purchasegroups() -> dict[str, Any]:
    ...
    """
    credits = STATE.credits()
    points_balance = STATE.fifa_points()
    settings = load_pack_settings()
    ...
    for priority, (pid_s, cfg0) in enumerate(
        ...
        offer: dict[str, Any] = {
            ...
            "visible": True,
            "isPurchaseable": credits >= price or (fifa_points > 0 and points_balance >= fifa_points),
...
def _ai_user_bids() -> dict[str, dict[str, Any]]:
    raw = STATE.get("ai_user_bids", {})
    return raw if isinstance(raw, dict) else {}


def _save_ai_user_bids(records: dict[str, dict[str, Any]]) -> None:
    STATE.set("ai_user_bids", records)


def _auction_bid_increment(value: int) -> int:
...
def _settle_ai_user_bid(trade_id: int) -> dict[str, Any] | None:
    """Finish one local-market bid and award the card exactly once."""
    records = _ai_user_bids()
    key = str(int(trade_id))
    record = records.get(key)
    if not isinstance(record, dict):
        return None
    now = now_s()
    # Repair wins created by v49-v51: those builds created the card in the
    # unassigned pile but exposed no working Claim action in Transfer Targets.
    if str(record.get("state", "")) == "won" and not bool(record.get("delivered", False)):
        won_item_id = int(record.get("wonItemId", 0) or 0)
        if won_item_id and STATE.update_item_pile(won_item_id, "club"):
            record["delivered"] = True
            record["deliveredAt"] = now
            records[key] = record
            _save_ai_user_bids(records)
            _watchlist_remove(int(trade_id))
            log.warning("AI AUCTION REPAIRED DELIVERY trade=%s item=%s pile=club", trade_id, won_item_id)
    # Roughly half of local auctions attract one AI opponent. It has a stable
    # hidden ceiling and responds after a short delay, allowing realistic
    # back-and-forth bidding without making every bargain impossible.
    if (
        str(record.get("state", "active")) == "active"
        and str(record.get("bidState", "highest")) == "highest"
        and bool(record.get("aiWillBid", False))
        and now < int(record.get("expiresAt", 0) or 0)
        and now >= int(record.get("nextAiBidAt", now + 1) or (now + 1))
    ):
        current = int(record.get("bid", 0) or 0)
        ceiling = int(record.get("aiCeiling", 0) or 0)
        ai_bid = min(ceiling, current + _auction_bid_increment(current))
        if ai_bid > current:
            refund = int(record.get("userBid", current) or 0)
            if refund > 0:
                STATE.add_credits(refund)
            record.update({
                "bid": ai_bid, "userBid": 0, "bidState": "outbid",
                "offerCount": int(record.get("offerCount", 1) or 1) + 1,
                "nextAiBidAt": 0,
            })
            records[key] = record
            _save_ai_user_bids(records)
            log.warning("AI AUCTION OUTBID trade=%s aiBid=%s refunded=%s ceiling=%s", trade_id, ai_bid, refund, ceiling)
    if str(record.get("state", "active")) == "active" and now >= int(record.get("expiresAt", 0) or 0):
        if str(record.get("bidState", "highest")) == "outbid":
            record["state"] = "lost"
            record["settledAt"] = now
            records[key] = record
            _save_ai_user_bids(records)
            log.warning("AI AUCTION LOST trade=%s finalBid=%s", trade_id, record.get("bid"))
            return record
        rid = int(record.get("resourceId", 0) or 0)
        # FIFA 15 PC does not expose a reliable Claim Won Item action for our
        # synthetic auction. Deliver directly to My Club instead.
        item = STATE.make_player_item(rid, "club")
        item["lastSalePrice"] = int(record.get("bid", 0) or 0)
        with STATE.lock:
            STATE.conn.execute(
                "UPDATE items SET data=? WHERE id=?",
                (json.dumps(item, separators=(",", ":"), ensure_ascii=False), int(item["id"])),
            )
            STATE.conn.commit()
        record["state"] = "won"
        record["wonItemId"] = int(item["id"])
        record["settledAt"] = now
        record["delivered"] = True
        record["deliveredAt"] = now
        records[key] = record
        ...
        _watchlist_remove(int(trade_id))
        log.warning(
            "AI AUCTION WON AND DELIVERED trade=%s rid=%s bid=%s item=%s pile=club",
            trade_id, rid, record.get("bid"), item.get("id"),
        )
    return record


def _ai_user_bid_listing(trade_id: int) -> dict[str, Any] | None:
    record = _settle_ai_user_bid(int(trade_id))
    if not record:
        return None
    rid = int(record.get("resourceId", 0) or 0)
    slot = int(record.get("slot", 0) or 0)
    listing = _market_listing_for_resource(rid, slot)
    active = str(record.get("state", "active")) == "active"
    listing.update({
        "tradeId": int(trade_id),
        "currentBid": int(record.get("bid", 0) or 0),
        "offers": max(1, int(record.get("offerCount", 1) or 1)),
        "watched": True,
        "bidState": str(record.get("bidState", "highest") or "highest"),
        "tradeState": "active" if active else "closed",
        "expires": max(0, int(record.get("expiresAt", 0) or 0) - now_s()) if active else 0,
    })
    return listing

def _watchlist_ids() -> list[int]:
    raw = STATE.get("watchlist_trade_ids", [])
...
def _watchlist_add(trade_id: int) -> None:
    ids = _watchlist_ids()
    trade_id = int(trade_id)
    if trade_id not in ids:
        ids.append(trade_id)
    STATE.set("watchlist_trade_ids", ids[-50:])


def _watchlist_remove(trade_id: int) -> None:
    trade_id = int(trade_id)
    STATE.set("watchlist_trade_ids", [x for x in _watchlist_ids() if x != trade_id])


def _watchlist_listing(trade_id: int) -> dict[str, Any] | None:
    bid_listing = _ai_user_bid_listing(int(trade_id))
    if bid_listing is not None:
        return bid_listing
    recent = _RECENT_AI_TRADE_STATUS.get(int(trade_id))
    if recent is not None:
        listing = dict(recent.get("auction") or {})
        listing["watched"] = True
        return listing
    decoded = _market_decode_ai_trade(int(trade_id))
    if decoded:
        listing = _market_listing_for_resource(*decoded)
        listing["watched"] = True
        return listing
    row = STATE.user_auction(int(trade_id))
    if not row:
        return None
    listing = _user_auction_payload(row)
    listing["watched"] = True
    return listing


_AI_CARD_OFFER_INDEX: list[tuple[int, int]] | None = None


def _ai_card_offer_resource(target_value: int, seed: int, excluded_rid: int = 0) -> tuple[int, int] | None:
    """Choose a deterministic card worth at least the requested local value."""
    global _AI_CARD_OFFER_INDEX
    if _AI_CARD_OFFER_INDEX is None:
        _AI_CARD_OFFER_INDEX = sorted(
            (STATE.market_reference_price(int(rid)), int(rid)) for rid in PLAYER_DB
        )
    eligible = [pair for pair in _AI_CARD_OFFER_INDEX if pair[0] >= int(target_value) and pair[1] != int(excluded_rid)]
    if not eligible:
        eligible = [pair for pair in _AI_CARD_OFFER_INDEX if pair[1] != int(excluded_rid)]
    if not eligible:
        return None
    # Pick among the closest twenty qualifying cards so offers vary while never
    # dropping below the listing's stored valuation.
    window = eligible[:20]
    return window[int(seed) % len(window)]


def _ai_card_offer_meta(row: dict[str, Any]) -> dict[str, Any]:
    try:
...
def _ai_card_offer_id(row: dict[str, Any]) -> int:
    meta = _ai_card_offer_meta(row)
    sequence = max(0, int(meta.get("cardOfferSequence", 0) or 0))
    trade_id = int(row.get("trade_id", 0) or 0)
    return 890000000 + ((trade_id + sequence * 1000003) % 100000000)


def _ai_card_offer_items(row: dict[str, Any]) -> list[dict[str, Any]]:
    current_bid = int(row.get("current_bid", 0) or 0)
    if current_bid <= 0 or str(row.get("state", "")) != "active":
        return []
    offer_meta = _ai_card_offer_meta(row)
    # A rejected swap starts a short cooldown. Its independent coin bid remains.
    if int(offer_meta.get("cardOfferNextAt", 0) or 0) > now_s():
        return []
    trade_id = int(row.get("trade_id", 0) or 0)
    own_rid = int(row.get("resource_id", 0) or 0)
    sequence = max(0, int(offer_meta.get("cardOfferSequence", 0) or 0))
    global _AI_CARD_OFFER_INDEX
    if _AI_CARD_OFFER_INDEX is None:
        _AI_CARD_OFFER_INDEX = sorted(
            (STATE.market_reference_price(int(rid)), int(rid)) for rid in PLAYER_DB
        )
    available = [pair for pair in _AI_CARD_OFFER_INDEX if pair[0] > 0 and pair[1] != own_rid]
    selected_cards: list[tuple[int, int]] = []
    remaining = current_bid
    seed = trade_id ^ own_rid ^ (sequence * 2654435761)
    # Each successive offer deterministically varies from one to four cards.
    # Any value the cards cannot cover is supplied as offer credits below, just
    # like a real mixed card-and-coin trade proposal.
    desired_cards = 1 + (seed % 4)
    # Native FIFA 15 trade offers support up to four player cards. Build a
    # combined-value offer instead of falling back to cheap bronze cards when a
    # single card cannot match an elite listing's value.
    for slot in range(desired_cards):
        if remaining <= 0 or not available:
            break
        slots_left = desired_cards - slot
        needed = max(1, (remaining + slots_left - 1) // slots_left)
        qualifying = [pair for pair in available if pair[0] >= needed]
        if qualifying:
            window = qualifying[:20]
            chosen = window[(seed + slot * 7919) % len(window)]
        else:
            # Nothing individually reaches this share: use one of the very best
            # remaining cards, never the old cheapest-card/bronze fallback.
            window = available[-min(10, len(available)):]
            chosen = window[(seed + slot * 7919) % len(window)]
        selected_cards.append(chosen)
        available = [pair for pair in available if pair[1] != chosen[1]]
        remaining -= int(chosen[0])

    offered: list[dict[str, Any]] = []
    for slot, (value, resource_id) in enumerate(selected_cards):
        virtual_id = 880000000000 + (trade_id % 1000000000) * 10 + slot
        native = _native_player_item(_virtual_market_item(resource_id, virtual_id))
        native.update({
            "pile": 5, "itemState": "free", "lastSalePrice": int(value),
            "swap": True, "tradeId": trade_id, "tradeIdStr": str(trade_id),
            "tradeOwner": False,
        })
        offered.append(native)
    return offered
...
def _market_price_for_resource(resource_id: int, slot: int = 0) -> tuple[int, int, int]:
    ...
    meta = player_meta(rid)
    is_legend = int(meta.get("rareflag", 0) or 0) == 12
    factor = (0.94 + rng.random() * 0.14) if is_legend else (0.88 + rng.random() * 0.24)
    ...
    # Local no-bid auctions deliberately open well below Buy Now, allowing a
    # patient player to win bargains rather than paying the guide price.
    # Icons should retain premium value even in no-bid auctions.  They can
    # still be won below Buy Now, but no longer start at only 25% of value.
    opening_factor = (0.55 + rng.random() * 0.23) if is_legend else (0.25 + rng.random() * 0.30)
    starting = max(150, int(round((buy_now * opening_factor) / step) * step))
    starting = min(starting, max(150, buy_now - step))
...
def _market_listing_for_resource(resource_id: int, slot: int = 0) -> dict[str, Any]:
    rid = int(resource_id)
    # Reserve two decimal digits for the copy index.  The old one-digit scheme
    # clamped copies 10-19 back to slot 9 when ai_market_copies_per_card was
    # set to 20. Duplicate trade/item IDs then corrupted FIFA's card cache and
    # made Icon listings intermittently render as raw bronze base cards.
    slot = max(0, min(99, int(slot)))
    starting, buy_now, seed = _market_price_for_resource(rid, slot)
    trade_id = 700000000000 + rid * 100 + slot
    expires = 240 + (seed % 3300)
    # Seed a realistic live market: some auctions already have AI interest,
    # while the rest remain no-bid bargains. Keep values deterministic so a
    # refresh does not randomly erase or change the visible current bid.
    interest_seed = int(hashlib.sha1(f"market-interest:{rid}:{slot}".encode()).hexdigest()[:8], 16)
    has_ai_bid = (interest_seed % 100) < 38
    if has_ai_bid:
        step = _auction_bid_increment(starting)
        bid_steps = 1 + ((interest_seed >> 8) % 8)
        current_bid = min(max(starting, buy_now - step), starting + bid_steps * step)
        current_bid = min(current_bid, max(starting, int(buy_now * 0.72)))
        offer_count = 1 + ((interest_seed >> 16) % 3)
    else:
        current_bid = 0
        offer_count = 0
    item = _virtual_market_item(rid, 500000000000 + rid * 100 + slot)
    return {
        "tradeId": trade_id,
        "itemData": _native_player_item(item),
        "tradeState": "active", "bidState": "none", "watched": False,
        "startingBid": starting, "currentBid": current_bid, "buyNowPrice": buy_now,
        "offers": offer_count, "expires": expires, "confidenceValue": 100,
...
def _market_decode_ai_trade(trade_id: int) -> tuple[int, int] | None:
    ...
    if v < 0: return None
    rid, slot = divmod(v, 100)
    if rid in PLAYER_DB and 0 <= slot <= 99:
...
def _market_search(query: dict[str, list[str]]) -> tuple[list[dict[str, Any]], int]:
    ...
    # Keep any configured card cap deterministic before expanding each player
    # into its concurrent auction copies.
    ...
    # FIFA 15's search request does not reliably transmit the selected sort
    # option. Return every result set in genuine Ending Soon order: remaining
    # seconds ascending, then the cheaper BIN for deterministic ties. Build only
    # lightweight tuples here and materialise the requested page afterwards.
    ending_order: list[tuple[int, int, int, int]] = []
    for slot in range(copies):
        for rid in candidates:
            _starting, bin_price, seed = _market_price_for_resource(rid, slot)
            if min_buy and bin_price < min_buy: continue
            if max_buy and bin_price > max_buy: continue
            remaining = 240 + (seed % 3300)
            ending_order.append((remaining, bin_price, rid, slot))
    ending_order.sort()
    total = len(ending_order)
    page = [
        _market_listing_for_resource(rid, slot)
        for _remaining, _bin, rid, slot in ending_order[start:start + num]
    ]
    return page, total
...
def _user_auction_payload(row: dict[str, Any]) -> dict[str, Any]:
    ...
    card_offer_active = bool(
        current_bid > 0
        and db_state == "active"
        and int(_ai_card_offer_meta(row).get("cardOfferNextAt", 0) or 0) <= now_s()
    )
    ...
    payload = {
        ...
        # FIFA displays this field as the number of player trade offers, not as
        # the presence of a separate coin bid.
        "offers": 1 if card_offer_active else 0,
        "offerReceived": card_offer_active,
        "watched": False,
        "bidState": "none",
        "tradeOwner": True,
    ...
    if card_offer_active:
        payload["offerId"] = _ai_card_offer_id(row)
        payload["offerState"] = "active"
    if db_state == "sold":
...
def route_fut(method: str, raw_path: str, headers: dict[str, str], body: bytes) -> tuple[int, dict[str, str], bytes]:
    ...
    # Static identity fallback for the activated FIFA 15 Pelé definition.
    # The embedded FUT client requests this independently of the owned-item
    # response whenever its local static catalogue has no usable entry.
    if method == "GET" and re.search(r"/fut/items/web/225365\.json$", low):
        static_player = {
            "FirstName": "Edson",
        ...
        return 200, {"Cache-Control": "no-store"}, json_bytes(static_player)

    pele_portrait = (
        re.search(r"/fut/items/images/(?:mobile/)?portraits/225365\.png$", low)
        or re.search(r"/fut/playerheads/(?:mobile/)?single/p201551957\.png$", low)
    )
    if method == "GET" and pele_portrait:
        image_path = ROOT / "assets" / "players" / "pele.png"
        if image_path.exists():
            return 200, {"Content-Type": "image/png", "Cache-Control": "no-store"}, image_path.read_bytes()

    # Card-swap protocol capture. FIFA first marks each offered club item with
    # swap/tradeId through /item, then commits the offer through this route.
    # Keep this narrowly scoped so normal request logging remains readable.
    if re.fullmatch(r"/ut/game/fifa15/trade/\d+/offer", low) and method in ("POST", "PUT"):
        log.warning(
            "CARD TRADE OFFER COMMIT method=%s path=%s payload=%s raw=%r",
            method, raw_path, payload, body[:4000],
        )

    # Local-only companion endpoint used by the desktop Admin Panel. It never
    # grants more than one reward per calendar day and uses the normal pack
    # generator so duplicate and unassigned-item rules remain authoritative.
    if low == "/local-admin/rewards/daily":
        if method == "GET":
            return 200, {"Cache-Control": "no-store"}, json_bytes(STATE.daily_reward_status())
        if method == "POST":
            result = STATE.claim_daily_reward()
            return 200, {"Cache-Control": "no-store"}, json_bytes(result)
    ...
    if low == "/ut/game/fifa15/settings":
        return 200, {}, json_bytes({"configs": [
            ...
            {"type": "IsTradingEnabled", "value": "1"},
            {"type": "tradingEnabled", "value": "1"},
            {"type": "IsTradeOffersEnabled", "value": "1"},
            {"type": "tradeOffersEnabled", "value": "1"},
    ...
    if low == "/ut/game/fifa15/hub":
        ...
        watched = [x for x in (_watchlist_listing(tid) for tid in _watchlist_ids()) if x]
        return 200, {}, json_bytes({
            ...
            "watchlist": {
                "count": len(watched),
                "outbid": sum(1 for x in watched if str(x.get("bidState", "none")) == "outbid"),
                "winning": sum(1 for x in watched if str(x.get("bidState", "none")) == "highest"),
            },
            "squad": _native_squad(),
        })
    ...
    if low == "/ut/game/fifa15/club":
        ...
        page_items = [
            _native_player_item(x)
            if int(x.get("resourceId", 0) or 0) == 201551957 else x
            for x in page_items
        ]
    ...
    if low == "/ut/game/fifa15/purchased/items":
        if method == "POST":
            ...
            use_credits = bool(int(payload.get("useCredits", 1))) if isinstance(payload, dict) else True
            use_preorder = bool(int(payload.get("usePreOrder", 0))) if isinstance(payload, dict) else False
            if use_credits:
                if STATE.credits() < price:
                    return 200, {}, json_bytes({"duplicateItemIdList": [], "itemData": [], **_credits_payload()})
                STATE.add_credits(-price)
            else:
                points_price = max(0, int(get_pack_cfg(pack_id).get("fifa_points", 0) or 0))
                # FIFA 15 labels a custom promo pack with no points price as a
                # pre-order entitlement (useCredits=0,usePreOrder=1). It still
                # expects the pack contents in the POST response. The previous
                # handler returned an empty success, making the reveal briefly
                # start and immediately close.
                if points_price <= 0 and use_preorder:
                    log.warning("LOCAL PREORDER PACK accepted packId=%s without charge", pack_id)
                elif points_price <= 0 or STATE.fifa_points() < points_price:
                    return 200, {}, json_bytes({"duplicateItemIdList": [], "itemData": [], **_credits_payload()})
                else:
                    STATE.add_fifa_points(-points_price)
            ...
            ordered_pack_items = [
                _native_player_item(x)
                if int(x.get("resourceId", 0) or 0) == 201551957
                else _native_pack_item(x)
                for x in pack.get("itemData", [])
            ]
            real_resources = [x.get("resourceId") for x in ordered_pack_items]
            # A one-card pack has no ordering problem to solve. Preserve its
            # real special resourceId; aliasing it converted Legend rarity 12
            # into a fake rare-gold definition and produced a NOT FOUND card.
            if len(ordered_pack_items) > 1:
        ...
        def _unassigned_reveal_sort_key(item):
            ...
            try:
                ...
                rid = 0
            # Players first, then true OVR descending; rarity breaks ties.
            is_player = str(item.get("itemType", "player")) == "player"
            return (0 if is_player else 1, -rating, -rareflag, -rid)
    ...
    watchlist_item = re.fullmatch(r"/ut/game/fifa15/watchlist/(\d+)", low)
    if low == "/ut/game/fifa15/watchlist" or watchlist_item:
        def requested_watch_trade_id() -> int:
            if watchlist_item:
                return int(watchlist_item.group(1))
            for key, values in query.items():
                if key.lower() in ("tradeid", "tradeids") and values:
                    try:
                        return int(str(values[0]).split(",")[0])
                    except Exception:
                        pass
            if isinstance(payload, dict):
                candidates = payload.get("auctionInfo", payload.get("tradeId", payload.get("id", [])))
                if isinstance(candidates, list) and candidates:
                    candidates = candidates[0]
                if isinstance(candidates, dict):
                    candidates = candidates.get("tradeId", candidates.get("id", 0))
                try:
                    return int(candidates or 0)
                except Exception:
                    pass
            return 0

        if method == "GET":
            start = max(0, int(query.get("offset", query.get("start", [0]))[0] or 0))
            count = max(1, min(50, int(query.get("count", query.get("num", [50]))[0] or 50)))
            all_listings = [x for x in (_watchlist_listing(tid) for tid in _watchlist_ids()) if x]
            listings = all_listings[start:start + count]
            return 200, {}, json_bytes({
                "auctionInfo": listings, "itemData": [], "total": len(all_listings), "count": len(all_listings),
                "winning": sum(1 for x in all_listings if str(x.get("bidState", "none")) == "highest"),
                "outbid": sum(1 for x in all_listings if str(x.get("bidState", "none")) == "outbid"),
                "credits": STATE.credits(),
            })
        trade_id = requested_watch_trade_id()
        if method in ("POST", "PUT") and trade_id:
            _watchlist_add(trade_id)
            listing = _watchlist_listing(trade_id)
            log.warning("TRANSFER TARGET ADD trade=%s found=%s", trade_id, bool(listing))
            return 200, {"Cache-Control": "no-store"}, json_bytes({
                "success": True, "auctionInfo": [listing] if listing else [], **_credits_payload(),
            })
        if method == "DELETE" and trade_id:
            _watchlist_remove(trade_id)
            log.warning("TRANSFER TARGET REMOVE trade=%s", trade_id)
            return 200, {"Cache-Control": "no-store"}, json_bytes({"success": True, **_credits_payload()})
        if method in ("POST", "PUT", "DELETE"):
            log.warning("TRANSFER TARGET request missing trade id method=%s path=%s payload=%s", method, raw_path, payload)
            return 200, {}, json_bytes({"success": False, "auctionInfo": [], **_credits_payload()})
    ...
    if low in (
        ...
        # Unlock the full Local FUT range in FIFA's listing form. The guide is
        # still logged and used by the AI buyer, but no longer restricts what
        # the user may ask for their own card.
        min_price = 150
    ...
    # FIFA 15 requests this detail route when Y is pressed on a transfer-list
    # entry. The captured card-swap commit is {bid, itemData}; the inverse GET
    # uses that same pair to display the incoming offer.
    native_offer_detail = re.match(r"^/ut/game/fifa15/trade/(\d+)/offer$", low)
    if native_offer_detail and method == "GET":
        trade_id = int(native_offer_detail.group(1))
        STATE.simulate_ai_sales()
        row = STATE.user_auction(trade_id)
        owner_listing = _user_auction_payload(row) if row else None
        offered_items = _ai_card_offer_items(row) if row else []
        offered_credits = max(
            0,
            int(row.get("current_bid", 0) or 0)
            - sum(int(item.get("lastSalePrice", 0) or 0) for item in offered_items),
        ) if row and offered_items else 0
        offer_rows: list[dict[str, Any]] = []
        # GetOffers stores its `id` member as a 32-bit integer. Auction trade
        # IDs are 64-bit, so derive a separate stable, positive offer ID.
        offer_id = _ai_card_offer_id(row) if row else 890000000 + (trade_id % 100000000)
        for offered_item in offered_items:
            offered_item["offerId"] = offer_id
            offered_item["offerState"] = "active"
        if offered_items:
            offer_rows.append({
                "id": offer_id,
                "tradeId": trade_id,
                ...
                "bid": 0,
                "offers": 1,
                "expires": int(owner_listing.get("expires", 0) or 0) if owner_listing else 0,
                "sellerName": "Local Trader",
                "sellerEstablished": 2014,
                "sellerId": 9900000001,
                "itemData": offered_items[0],
            })
        # Runtime mapping of the retail PC GetOffers parser gives this exact
        # contract: offer[] -> {credits, id, itemList:{itemData[]}}.
        trade_offer = {
            "credits": offered_credits,
            "id": offer_id,
            "itemList": {"itemData": offered_items},
        }
        response = {
            "offer": [trade_offer] if offered_items else [],
            "total": 1 if offered_items else 0,
            **_credits_payload(),
        }
        log.warning("TRADE OFFER NATIVE RESPONSE trade=%s found=%s offers=%s currentBid=%s offeredItems=%s",
                    trade_id, bool(owner_listing),
                    int(owner_listing.get("offers", 0) or 0) if owner_listing else 0,
                    int(owner_listing.get("currentBid", 0) or 0) if owner_listing else 0,
                    [(x.get("id"), x.get("resourceId"), x.get("rating")) for x in offered_items])
        return 200, {"Cache-Control": "no-store"}, json_bytes(response)

    # Native trade-offer decision route. FIFA sends exactly
    # {"offerState":"Accept"} or {"offerState":"Reject"} here, then refreshes
    # Purchased Items and the Transfer List. An accepted card swap consumes the
    # user's listed card and places the offered card in Unassigned Items.
    offer_decision = re.match(r"^/ut/game/fifa15/trade/(\d+)/offer/(\d+)$", low)
    if offer_decision and method in ("PUT", "POST"):
        trade_id = int(offer_decision.group(1))
        offer_id = int(offer_decision.group(2))
        action = str(payload.get("offerState", "") if isinstance(payload, dict) else "").strip().lower()
        row = STATE.user_auction(trade_id)
        expected_offer_id = _ai_card_offer_id(row) if row else 890000000 + (trade_id % 100000000)
        offered_items = _ai_card_offer_items(row) if row else []
        offered_credits = max(
            0,
            int(row.get("current_bid", 0) or 0)
        ...
        if not row or offer_id != expected_offer_id or action not in ("accept", "reject"):
            log.warning("TRADE OFFER DECISION rejected trade=%s offer=%s action=%s found=%s",
                        trade_id, offer_id, action, bool(row))
            return 200, {}, json_bytes({"error": "NO_OFFER_EXISTS", "offerState": "Invalid"})

        if action == "accept":
            listed_item_id = int(row.get("item_id", 0) or 0)
            offered_rids = [int(item.get("resourceId", 0) or 0) for item in offered_items]
            if not offered_rids or any(not rid for rid in offered_rids):
                return 200, {}, json_bytes({"error": "NO_OFFER_EXISTS", "offerState": "Invalid"})
            # RLock makes this one local transaction boundary from the request's
            # perspective; make_player_item persists the replacement first, then
            # the outgoing listing and its owned item are removed.
            with STATE.lock:
                received_items = []
                for offered, offered_rid in zip(offered_items, offered_rids):
                    received = STATE.make_player_item(offered_rid, "unassigned")
                    received["lastSalePrice"] = max(1, int(offered.get("lastSalePrice", 0) or 0))
                    received["pile"] = "unassigned"
                    received["itemState"] = "free"
                    STATE.conn.execute(
                        "UPDATE items SET pile=?,data=? WHERE id=?",
                        ("unassigned", json.dumps(received), int(received["id"])),
                    )
                    received_items.append(received)
                STATE.conn.execute("DELETE FROM auctions WHERE trade_id=?", (trade_id,))
                if listed_item_id:
                    STATE.conn.execute("DELETE FROM items WHERE id=?", (listed_item_id,))
                STATE.conn.commit()
            if offered_credits:
                STATE.add_credits(offered_credits)
            _watchlist_remove(trade_id)
            log.warning(
                "TRADE OFFER ACCEPTED trade=%s offer=%s sentItem=%s receivedItems=%s receivedRids=%s credits=%s",
                trade_id, offer_id, listed_item_id,
                [int(item.get("id", 0)) for item in received_items], offered_rids, offered_credits,
            )
            # The retail client already advances to New Items on an empty 200
            # response. Keep that proven response contract; the following
            # /purchased/items request supplies the newly transferred card.
            return 200, {"Cache-Control": "no-store"}, json_bytes({})

        # Rejection clears only the player-swap offer. The current_bid column is
        # the auction's independent coin bid and must remain untouched.
        try:
            meta = json.loads(row.get("data", "{}") or "{}")
        except Exception:
            meta = {}
        if not isinstance(meta, dict):
            meta = {}
        sequence = max(0, int(meta.get("cardOfferSequence", 0) or 0)) + 1
        meta.pop("cardOfferRejected", None)
        meta["cardOfferSequence"] = sequence
        meta["cardOfferNextAt"] = now_s() + 30
        with STATE.lock:
            STATE.conn.execute(
                "UPDATE auctions SET data=? WHERE trade_id=?",
                (json.dumps(meta, separators=(",", ":")), trade_id),
            )
            STATE.conn.commit()
        log.warning("TRADE OFFER REJECTED trade=%s offer=%s", trade_id, offer_id)
        return 200, {"Cache-Control": "no-store"}, json_bytes({})
    ...
    if offer_match and method in ("PUT", "POST"):
        ...
        # Every bid, Buy Now, or card-swap offer belongs in Transfer Targets so
        # the native screen can track its outcome even if FIFA did not send a
        # separate watchList request first.
        _watchlist_add(trade_id)
        ...
        if amount >= buy_now and buy_now > 0:
            bid_records = _ai_user_bids()
            prior_bid_record = bid_records.get(str(trade_id)) if isinstance(bid_records.get(str(trade_id)), dict) else {}
            reserved_bid = (
                int(prior_bid_record.get("userBid", prior_bid_record.get("bid", 0)) or 0)
                if str(prior_bid_record.get("state", "")) == "active"
                and str(prior_bid_record.get("bidState", "highest")) == "highest"
                else 0
            )
            remaining_price = max(0, buy_now - reserved_bid)
            if STATE.credits() < remaining_price:
            ...
            STATE.add_credits(-remaining_price)
            if prior_bid_record:
                prior_bid_record["state"] = "bought"
                prior_bid_record["settledAt"] = now_s()
                bid_records[str(trade_id)] = prior_bid_record
                _save_ai_user_bids(bid_records)
        ...
        # Persist a true local bid. Coins are reserved immediately and the card
        # is delivered to New Items when this listing expires if the user is
        # still the highest bidder.
        starting_bid = int(listing.get("startingBid", 0) or 0)
        bid_amount = max(starting_bid, amount)
        records = _ai_user_bids()
        key = str(trade_id)
        existing = records.get(key) if isinstance(records.get(key), dict) else {}
        current_bid = int(existing.get("bid", listing.get("currentBid", 0)) or 0)
        previous_bid = int(existing.get("userBid", current_bid) or 0) if str(existing.get("bidState", "highest")) == "highest" else 0
        if str(existing.get("state", "active")) != "active" and existing:
            return 200, {}, json_bytes({"auctionInfo": [_ai_user_bid_listing(trade_id)], "error": "TRADE_EXPIRED", **_credits_payload()})
        if current_bid > 0 and (not existing or str(existing.get("bidState", "highest")) == "outbid"):
            bid_amount = max(bid_amount, current_bid + _auction_bid_increment(current_bid))
        additional = max(0, bid_amount - previous_bid)
        if additional <= 0 and previous_bid > 0:
            bid_amount = previous_bid
        elif STATE.credits() < additional:
            return 200, {}, json_bytes({"auctionInfo": [listing], "error": "INSUFFICIENT_FUNDS", **_credits_payload()})
        else:
            STATE.add_credits(-additional)
        expires_at = int(existing.get("expiresAt", 0) or 0)
        if expires_at <= now_s():
            expires_at = now_s() + max(15, int(listing.get("expires", 300) or 300))
        competition_seed = int(hashlib.sha1(f"ai-bid:{trade_id}:{rid}".encode()).hexdigest()[:8], 16)
        ai_ceiling = int(existing.get("aiCeiling", 0) or 0)
        if ai_ceiling <= 0:
            ai_ceiling = max(starting_bid, int(buy_now * (0.55 + (competition_seed % 41) / 100.0)))
        records[key] = {
            "tradeId": trade_id, "resourceId": rid, "slot": slot,
            "bid": bid_amount, "created": int(existing.get("created", now_s()) or now_s()),
            "userBid": bid_amount, "bidState": "highest",
            "offerCount": int(existing.get("offerCount", listing.get("offers", 0)) or 0) + 1,
            "expiresAt": expires_at, "state": "active",
            "aiWillBid": bool(existing.get("aiWillBid", (competition_seed % 100) < 55)),
            "aiCeiling": ai_ceiling,
            "nextAiBidAt": now_s() + 10 + (competition_seed % 26),
        }
        _save_ai_user_bids(records)
        listing = _ai_user_bid_listing(trade_id) or listing
        log.warning("AI AUCTION BID trade=%s rid=%s bid=%s reserved=%s expiresAt=%s", trade_id, rid, bid_amount, additional, expires_at)
        return 200, {}, json_bytes({"success": True, "auctionInfo": [listing], "duplicateItemIdList": [], **_credits_payload()})
    ...
    if low in ("/ut/game/fifa15/auctionhouse/status", "/ut/game/fifa15/trade/status"):
        ...
        for token in raw_ids.split(","):
            ...
            if recent is not None:
                ...
                continue
            bid_listing = _ai_user_bid_listing(tid)
            if bid_listing is not None:
                out.append(bid_listing)
                continue
    ...
    if low == "/ut/game/fifa15/item" and method in ("PUT", "POST"):
        ...
        for change in data_list:
            ...
            if bool(change.get("swap")) or change.get("tradeId") is not None:
                log.warning("CARD TRADE OFFER ITEM payload=%s", change)
        ...
        if rid:
            for vrid, vmeta in PLAYER_DB.items():
                ...
                if asset != rid and int(vrid) != rid:
                    continue
                variants.append({
                    "id": 0, "assetId": asset, "definitionId": int(vrid),
                    "resourceId": int(vrid), "itemType": "player",
